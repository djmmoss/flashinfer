...
Architecture -- one coherent design, one GPU kernel body, one launch:
  ...
  * One CTA owns one sequence/head pair. Inside a CTA the tile is
    `NT = (D/R) * KQ` threads = (D/R) row-groups x KQ k-lane-groups; thread
    (rg, kq) owns rows `rg*R ..` and, inside each of them, the k positions
    `jj*(4*KQ) + kq*4 + m`.  Every gmem touch of the state is a fully coalesced
    128-bit vector op and every reduction over k is a log2(KQ)-step butterfly.
  * Value rows are independent in the delta rule. The CTA reduces its complete
    head locally for gated RMSNorm, avoiding cross-CTA cache dependencies.
  * (R, KQ) are selected from the launch geometry while preserving this single
    CTA-per-head architecture.
  * The front-end (width-4 causal depthwise conv + SiLU, L2 norms of q/k, the
    per-channel decay, beta) is computed by the same threads into a small
...
def _required_smem_bytes(T, R, KQ, NCH):
...
def _kda_kernel(
    ...
    slot0 = gIdx[n, 0]
    livef = cute.min(
        cute.max(slot0.to(cutlass.Float32), cutlass.Float32(0.0)), cutlass.Float32(1.0)
    )
    src = cute.max(slot0, cutlass.Int32(0))
    live = livef > 0.5
    ...
    if cutlass.const_expr(NCH == 1):
        ...
        for blk in cutlass.range_constexpr(NEP):
            ...
            pvv = row0 + pidx % RB
            ogv.append(
                _fsigmoid(gOG[n * T + pidx // RB, h * _D + pvv].to(cutlass.Float32))
            )
            nwv.append(gNW[pvv])
    ...
    for cb in cutlass.range_constexpr((4 * _D + NT - 1) // NT):
        ...
        if c < 3 * _D:
            ...
            cd = c % _D
            chan = ci * dim + h * _D + cd
            hh0 = gConv[src, chan, 0].to(cutlass.Float32)
            hh1 = gConv[src, chan, 1].to(cutlass.Float32)
            hh2 = gConv[src, chan, 2].to(cutlass.Float32)
            ...
            for t in cutlass.range_constexpr(T):
                xv = gX[n * T + t, chan].to(cutlass.Float32)
                ...
                if live:
                    if mine:
                        if slots[t] > 0:
                            gConv[slots[t], chan, 0] = hh0.to(cutlass.BFloat16)
                            gConv[slots[t], chan, 1] = hh1.to(cutlass.BFloat16)
                            gConv[slots[t], chan, 2] = hh2.to(cutlass.BFloat16)
        elif c < 4 * _D:
            dd = c - 3 * _D
            a_h = _fexp(gAlog[h])
            dtb = gDtb[h * _D + dd]
            for t in cutlass.range_constexpr(T):
                gi = gGate[n * T + t, h * _D + dd].to(cutlass.Float32) + dtb
                sDec[t * _D + dd] = _fexp(lb * _fsigmoid(a_h * gi))

    if tid < T:
        sBeta[tid] = _fsigmoid(gBeta[n * T + tid, h].to(cutlass.Float32))
    ...
    for nb in cutlass.range_constexpr((T * 32 + NT - 1) // NT):
        ...
        if idx < T * 32:
            ...
            cute.autovec_copy(_svec(a_qkv, t * (3 * _D) + l * 4, cutlass.BFloat16), nq4)
            cute.autovec_copy(
                _svec(a_qkv, t * (3 * _D) + _D + l * 4, cutlass.BFloat16), nk4
            )
            ...
            if l == 0:
                sRed[t * 4] = cute.rsqrt(sqq + _L2_EPS, approx=True, ftz=True) * scale
    ...
    for ch in cutlass.range_constexpr(NCH):
        ...
        for t in cutlass.range_constexpr(T):
            ...
            if live:
                if slots[t] > 0:
                    dst_base = p_state + slots[t].to(cutlass.Int64) * (state_page * 4)
                    for r in cutlass.range_constexpr(R):
    ...
    if cutlass.const_expr(SPLIT > 1):
        ...
        if tid < T:
            ...
            for p in cutlass.range_constexpr(SPLIT):
                cute.arch.store_async_dsmem(slot, tot, mbar, p)
    ...
    if cutlass.const_expr(NCH == 1):
        for blk in cutlass.range_constexpr(NEP):
            idx = blk * NT + tid
            if idx < T * RB:
                gOut[n * T + idx // RB, h * _D + row0 + idx % RB] = (
                    sOut[idx] * sTot[idx // RB] * nwv[blk] * ogv[blk]
                ).to(cutlass.BFloat16)
    else:
        for blk in cutlass.range_constexpr(NEP):
            idx = blk * NT + tid
            if idx < T * RB:
                ete = idx // RB
                evv = row0 + idx % RB
                eog = _fsigmoid(gOG[n * T + ete, h * _D + evv].to(cutlass.Float32))
                gOut[n * T + ete, h * _D + evv] = (
...
def _kda_launch(
    ...
    gX = _t(BF, p_x, (nt, 3 * dim), (x_row_stride, 1))
    gW = _t(F32, p_w, (3, _W, dim), (_W * dim, dim, 1))
    gConv = m_conv
    gGate = _t(BF, p_gate, (nt, dim), (dim, 1))
    gBeta = _t(BF, p_beta, (nt, H), (beta_row_stride, 1))
    gAlog = _t(F32, p_alog, (H,), (1,))
    gDtb = _t(F32, p_dtb, (dim,), (1,))
    gIdx = _t(I32, p_idx, (N, T), (T, 1))
    gOG = _t(BF, p_og, (nt, dim), (output_gate_row_stride, 1))
    gNW = _t(F32, p_nw, (_D,), (1,))
    gOut = _t(BF, p_out, (nt, dim), (dim, 1))
...
def _get_compiled_kernel(
    ...
    fake_inputs = (
        ...
        _compact_fake((1, nt, H, _D), cutlass.BFloat16),
        cute.runtime.make_fake_tensor(
            cutlass.BFloat16,
            (1, nt, H),
    ...
    kernel_name = (
        ...
        f"_r{R}_kq{KQ}_bpm{bpm}_nch{NCH}"
    )
...
def _run_fused_kda_decode_multitoken(
    ...
    required_smem = _required_smem_bytes(T, R, KQ, NCH)
...
def run_fused_kda_decode_multitoken(
    ...
    """Run packed fused KDA and write one cache checkpoint per token.

    Each row of ``state_indices`` must be either entirely positive or entirely
    non-positive. Non-positive rows produce zero output and do not mutate the
    cache. Destination stores are guarded per token as a final safety net.
    """
    ...
    if x.ndim != 2 or x.shape[0] != num_sequences * num_tokens:
        raise ValueError("x must have shape [N * T, 3 * H * 128]")
    ...
    if (
        conv_state.ndim != 3
        or conv_state.shape[1:] != (3 * hidden_size, _W - 1)
        or conv_state.stride(1) != 1
        or conv_state.stride(2) != 3 * hidden_size
    ):
        raise ValueError("conv_state must use the paged SD cache layout")
    if raw_gate.shape != (1, num_sequences * num_tokens, num_heads, _D):
        raise ValueError("raw_gate must have shape [1, N * T, H, 128]")
    if not raw_gate.is_contiguous():
        raise ValueError("raw_gate must be contiguous")
    if (
        raw_beta.shape != (1, num_sequences * num_tokens, num_heads)
        or raw_beta.stride(2) != 1
    ):
        raise ValueError("raw_beta must have shape [1, N * T, H]")
    ...
    if output_gate.shape != (num_sequences * num_tokens, num_heads, _D):
        raise ValueError("output_gate must have shape [N * T, H, 128]")
    ...
    expected_output_shape = (1, num_sequences * num_tokens, num_heads, _D)
    if output is None:
        output = torch.empty(
        ...
        if output.shape != expected_output_shape or not output.is_contiguous():
            raise ValueError("output must be contiguous with shape [1, N * T, H, 128]")
...
# Evaluation harness compatibility while the public dispatch is developed.
run = run_fused_kda_decode_multitoken
